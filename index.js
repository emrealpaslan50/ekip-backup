...
client.config = global.config = {
  prefix: ".",
"token" : "ODIxNjc0NjEzNDAxODQ1NzYw.YFHKHQ.aGvJUzbm5lMb3F6GboyKVKcg5p4",
  status1: "idle",
  status2: "idle",
  status3: "idle",
  footer: "NevşehirLii Kimse Baş Edemez",
  durum1: "Rage ❤ NevşehirLii",
  durum2: "NevşehirLii ❤ Rage",
  durum3: "By Developer NevşehirLii",
  durum4:"NevşehirLii Was Here?",
  owners: "797369628610265108",

  "rolveri":"848898294293069864",
  "kanalveri":"848898344079327242",
  guild: "694891272007319673",
  voiceChannel: "694891273156427803",
  logChannel: "848898376405352478"
};

mongoose.connect("mongodb+srv://mongo123:mongo9999@cluster0.qmdme.mongodb.net/myFirstDatabase?retryWrites=true&w=majority",
  ...
  if (guild) {
    guild.roles.cache
      ...
      .forEach(role => {
        ...
        RoleData.findOne(
          { guildID: client.config.guild, roleID: role.id },
          async (err, savedRole) => {
            if (!savedRole) {
    ...
    RoleData.find({ guildID: client.config.guild })
      .sort()
      .exec((err, roles) => {
        roles
          .filter(
            r =>
              ...
              Date.now() - r.time > 1000 * 60 * 10
          )
          .forEach(r => {
            //1 saatte bir alır. Süreyi değiştirebilirsiinz.
            RoleData.findOneAndDelete({ roleID: r.roleID });
          });
      });
    	client.channels.cache.get(global.config.rolveri).send(`Rol Veri Tabanı Yedeklendi`) 
      
    console.log(
      `[${client.moment(Date.now()).format("L")}] Rol veri tabanı düzenlendi!`
    );
  }
}
  ...
  if (guild) {
    guild.channels.cache
      ...
      .forEach(channel => {
        ...
        ChannelData.findOne(
          { guildID: client.config.guild, channelID: channel.id },
            ...
            if (!savedChannel) {
              if (channel.type === "voice") {
                ...
                let newChannelSchema = new ChannelData({
                  ...
                  topic: channel.topic
                    ? channel.topic
                    : "Bu kanal Backup botu tarafından kurtarıldı!",
                  permissionOverwrites: permissionss
                });
                newChannelSchema.save();
              }
              ...
              if (channel.type === "voice") {
                ...
                savedChannel.type = channel.type;
                savedChannel.time = Date.now();
                savedChannel.topic = channel.topic
                  ? channel.topic
                  : "Bu kanal Backup botu tarafından kurtarıldı!";
                savedChannel.permissionOverwrites = permissionss;
                savedChannel.save();
    ...
    ChannelData.find({ guildID: client.config.guild })
      .sort()
      .exec((err, channels) => {
        channels
          .filter(
    ...
    client.channels.cache.get(global.config.kanalveri).send(`Kanal Veri Tabanı Yedeklendi`) 
       
    console.log(
      `[${client.moment(Date.now()).format("L")}] Kanal veri tabanı düzenlendi!`
    );
  }
}
...
////////////-------------------////////////
 ...
 client.on("ready", async () => {
  client.user.setPresence({
    activity: { name: "NevşehirLii ❤ Rage" },
    status: "idle"
  });

    let NevşehirLii = client.channels.cache.get(global.config.voiceChannel);
    if(NevşehirLii) NevşehirLii.join().catch(err => console.error("Bot ses kanalına bağlanamadı!"));

    setInterval(() => {
        NevşehirLii.join()
    setRoleBackup();
    setChannelBackup();
  }, 1000 * 60 * 10); // 1 saat. 1000*60*60*1
});
...
client
  ...
  .then(awoken =>
    console.log(
      `${client.user.username} İsmi ile giriş yapıldı! Database I Online`
  
    )
      
  )

  .catch(err => console.log("Database I giriş yapamadı!"));
